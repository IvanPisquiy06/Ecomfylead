...
const nextButton = document.getElementById('q1-next');

nextButton.addEventListener('click', function() {
  // Get the radio buttons by name
  const radioButtons = document.getElementsByName('property_ownership');

  // Loop through the radio buttons to find the selected one
  for (let i = 0; i < radioButtons.length; i++) {
    if (radioButtons[i].checked) {
      // The value of the selected radio button is:
      const selectedOption = radioButtons[i].value;
...
const id = "B1DDBAA74AEB491BACBA57445338F6F6"
const key = "b53f0c10"
const apiHeader = {
